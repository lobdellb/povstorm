...
def run_binary(cmd, cwd):
    ...
    if result.returncode != 0:
        logging.info(
            f"run_binary: return code was --{result.returncode}-- with stdout --{result.stdout}-- and stderr --{result.stderr}--"
        )
...
def process_workunit():
    ...
    shared_resource_local_path = os.path.join(
        config.MOUNT_PATH, work_unit.bespoke_resource_gcs_prefix
    )
    ...
    if not returncode == 0:
        raise Exception(
            f"povstorm:process_workunit:call to cmd: failed with error code --{returncode}--, stderr --{stderr}--, and stdout --{stdout}--."
        )
    else:
        logging.info(
            f"povstorm:process_workunit:call to cmd: suceeded with stderr --{stderr}--, and stdout --{stdout}--."
        )
